...
export async function copyAsync(from, to, options = {}) {
    ...
    if (globResolved.length > 0) {
        ...
        multimatch(normalizedStoreFilePaths, patterns.map((p) => (path.isAbsolute(p) ? p : path.posix.join(normalize(fromBasePath), p))), options.storeMatchOptions).forEach((filePath) => {
