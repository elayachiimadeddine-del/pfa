...
export function globify(inputFilePath) {
    if (Array.isArray(inputFilePath)) {
        return inputFilePath.reduce((memo, pattern) => memo.concat(globify(pattern)), []);
    }
...
export function isBinary(filePath, newFileContents) {
    ...
    return ((fs.existsSync(filePath) && isBinaryFileSync(filePath)) || (newFileContents && isBinaryFileSync(newFileContents)));
}
